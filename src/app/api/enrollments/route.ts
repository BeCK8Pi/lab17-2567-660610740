...
export const GET = async (request:NextRequest) => {
  ...
  if((studentId&&courseNo)||(!studentId&&!courseNo))  return NextResponse.json(
      {
        ok: false,
  ...
  if (studentId) {
    ...
    for(const student of studentIdList){
      const foundStudent = DB.students.find((s) => s.studentId === student);
      if(foundStudent) students.push(foundStudent);
    }
...
export const DELETE = async (request:NextRequest) => {
  ...
  const foundStudent = DB.enrollments.find((s) => s.studentId === studentId);
  const foundCourse = DB.enrollments.find((c) => c.courseNo === courseNo);

  if(!foundStudent||!foundCourse) return NextResponse.json(
       {
         ok: false,
  ...
  const foundEnroll = DB.enrollments.findIndex((e) => e.courseNo === courseNo && e.studentId === studentId);   
